...
<div class="antialiased bg-gray-50 min-h-screen py-8 md:py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- [REFAKTOR] - Header Halaman, Progress Bar, dan Ringkasan dirombak total --}}
        {{-- 1. Header Card Oranye Standar --}}
        <div class="bg-orange-50 border border-orange-200 rounded-xl shadow-sm p-6 mb-6">
            <div class="flex items-center space-x-4">
                <div class="flex-shrink-0 bg-orange-100 p-3 rounded-full">
                    <svg class="w-6 h-6 text-orange-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                </div>
                <div class="flex-grow">
                    <h2 class="text-lg font-bold text-gray-900">Langkah 3: Hasil Pelayanan</h2>
                    <p class="text-base text-gray-600 mt-1">
                        Catat status penyelesaian, hasil, dan output dari pelayanan yang diberikan.
                    </p>
                </div>
            </div>
        </div>

        {{-- 2. Kartu Ringkasan Informasi --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            <div class="bg-white border border-gray-200 rounded-xl p-5">
                <p class="text-sm font-medium text-gray-500 mb-1">Klien</p>
                <p class="font-semibold text-base text-gray-800 truncate">{{ $pelayanan->nama_pelanggan }}</p>
        ...
        <form action="{{ route('pelayanan.storeHasil', $pelayanan->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                {{-- [REFAKTOR] - Kartu 1: Status & Hasil --}}
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-8">
                    <div class="flex items-start space-x-4">
                        <div class="p-2 bg-gray-100 rounded-lg flex-shrink-0">
                            <svg class="w-6 h-6 text-gray-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                        </div>
                        <div class="flex-grow">
    ...
    <div class="relative" @click.away="dropdownOpen = false">
        ...
        <div x-show="dropdownOpen" x-transition class="absolute mt-2 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                    @php
                        $statuses = ['Selesai', 'Selesai dengan tindak lanjut', 'Tidak dapat dipenuhi', 'Dibatalkan klien'];
                    @endphp
                    @foreach ($statuses as $status)
                        <div @click="selectedValue = '{{ $status }}'; selectedText = '{{ $status }}'; dropdownOpen = false;" 
                            class="text-gray-900 relative cursor-pointer select-none py-2 px-4 hover:bg-orange-50 hover:text-orange-700 transition">
                    ...
                    @endforeach
                                ...
                                <div>
                                    ...
                                    <textarea name="deskripsi_hasil" id="deskripsi_hasil" rows="4" class="w-full text-base rounded-lg border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 py-2.5 px-4 transition" required placeholder="Jelaskan secara detail hasil dari pelayanan yang diberikan..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {{-- [REFAKTOR] - Kartu 2: Output, Dokumen & Tindak Lanjut --}}
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-8" x-data="{ requiresFollowUp: false }">
                    <div class="flex items-start space-x-4">
                        <div class="p-2 bg-gray-100 rounded-lg flex-shrink-0">
                             <svg class="w-6 h-6 text-gray-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M3 4a1 1 0 011-1h12a1 1 0 011 1v12a1 1 0 01-1 1H4a1 1 0 01-1-1V4zm2 2a1 1 0 011-1h8a1 1 0 011 1v8a1 1 0 01-1 1H6a1 1 0 01-1-1V6z" clip-rule="evenodd"></path></svg>
                        </div>
                        <div class="flex-grow">
                             ...
                             <div class="space-y-8 mt-6">
                                 <div>
                                     ...
                                     <div class="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
                                         ...
                                         <div class="flex items-center"><input id="output_{{ Str::slug($output) }}" name="jenis_output[]" type="checkbox" value="{{ $output }}" class="h-4 w-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500"><label for="output_{{ Str::slug($output) }}" class="ml-2 block text-base text-gray-800">{{ $output }}</label></div>
                                         @endforeach
                                     </div>
                                 </div>
                                 <div>
                                 ...
                                 <div x-show="requiresFollowUp" x-transition class="space-y-6">
                                     <div>
                                         ...
                                         <input type="date" name="tanggal_tindak_lanjut" id="tanggal_tindak_lanjut" class="w-full md:w-1/2 text-base rounded-lg border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 py-2.5 px-4 transition">
                                     </div>
                                     <div>
                                         <label for="catatan_tindak_lanjut" class="block text-sm font-medium text-gray-700 mb-1.5">Catatan Tindak Lanjut</label>
                                         <textarea name="catatan_tindak_lanjut" id="catatan_tindak_lanjut" rows="3" class="w-full text-base rounded-lg border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 py-2.5 px-4 transition" placeholder="Jelaskan tindak lanjut yang diperlukan..."></textarea>
                                     </div>
                                 </div>
                             </div>
                        </div>
                 ...
                 {{-- [REFAKTOR] - Kartu 3: Catatan Tambahan --}}
                 <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-8">
                     <div class="flex items-start space-x-4">
                         <div class="p-2 bg-gray-100 rounded-lg flex-shrink-0">
                            <svg class="w-6 h-6 text-gray-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0113 3.414L16.586 7A2 2 0 0118 8.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 10a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm0-3a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm0-3a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1z" clip-rule="evenodd"></path></svg>
                         </div>
                         <div class="flex-grow">
                              ...
                              <div class="mt-6">
                                <label for="catatan_tambahan" class="block text-sm font-medium text-gray-700 mb-1.5 sr-only">Catatan Tambahan</label>
                                <textarea name="catatan_tambahan" id="catatan_tambahan" rows="3" class="w-full text-base rounded-lg border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 py-2.5 px-4 transition" placeholder="Catatan internal atau informasi tambahan..."></textarea>
                              </div>
                         </div>
                     </div>
                 </div>
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4">
                    ...
                    <button type="submit" class="w-full sm:w-auto px-8 py-3 bg-orange-500 text-white text-sm font-semibold rounded-lg hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                        <span>Haalaman Selanjutnya</span>
                    </button>
                </div>
            </div>
        </form>
